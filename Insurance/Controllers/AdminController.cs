...
namespace Insurance.Controllers
{
    ...
    public class AdminController : ControllerBase
    {
        ...
        public async Task<IActionResult> Register([FromBody] AdminModel model)
        {
            try
            {
                var admin = await _adminBL.RegisterAdminAsync(model);
                var response = new ResponseModel<Admin>
                {
                    Success = true,
                    Message = $"Hello {admin.FullName}, you have been successfully registered as an admin.",
                    Data = admin
                };
        ...
        public async Task<IActionResult> Login([FromBody] LoginModel model)
        {
            try
            {
                var token = await _adminBL.LoginAdminAsync(model);
        ...
        public async Task<IActionResult> ForgotPassword([FromBody] ForgotPassword model)
        {
            try
            {
                var result = await _adminBL.ForgotPasswordAsync(model);
                if (result)
                {
                    var response = new ResponseModel<string>
                    {
        ...
        public async Task<IActionResult> ResetPassword([FromBody] ResetPasswordModel model)
        {
            try
            {
                var result = await _adminBL.ResetPasswordAsync(model);
                if (result)
                {
                    var response = new ResponseModel<string>
                    {
