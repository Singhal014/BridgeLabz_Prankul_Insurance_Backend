...
namespace Insurance.Controllers
{
    ...
    public class AgentController : ControllerBase
    {
        ...
        public async Task<IActionResult> Register([FromBody] AdminModel model)
        {
            try
            {
                var agent = await _agentBL.RegisterAgentAsync(model);
                var response = new ResponseModel<Agent>
                {
                    Success = true,
                    Message = $"Hello {agent.FullName}, you have been successfully registered as an agent.",
                    Data = agent
                };
                _logger.LogInformation("Agent registered successfully: {Email}", model.Email);
                return Ok(response);
            }
        ...
        public async Task<IActionResult> Login([FromBody] LoginModel model)
        {
            try
            {
                var token = await _agentBL.LoginAgentAsync(model);
        ...
        public async Task<IActionResult> ForgotPassword([FromBody] ForgotPassword model)
        {
            try
            {
                var result = await _agentBL.ForgotPasswordAsync(model);
                if (result)
                {
                    var response = new ResponseModel<string>
                    {
        ...
        public async Task<IActionResult> ResetPassword([FromBody] ResetPasswordModel model)
        {
            try
            {
                var result = await _agentBL.ResetPasswordAsync(model);
                if (result)
                {
                    var response = new ResponseModel<string>
                    {
