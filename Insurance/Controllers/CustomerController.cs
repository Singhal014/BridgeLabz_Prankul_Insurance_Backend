...
namespace Insurance.Controllers
{
    ...
    public class CustomerController : ControllerBase
    {
        ...
        public async Task<IActionResult> Register([FromBody] CustomerRegistration model)
        {
            try
            {
                var customer = await _customerBL.RegisterCustomerAsync(model);
                var response = new ResponseModel<Customer>
                {
                    Success = true,
                    Message = $"Hello {customer.FullName}, you have been successfully registered as a customer.",
                    Data = customer
                };
        ...
        public async Task<IActionResult> Login([FromBody] LoginModel model)
        {
            try
            {
                var token = await _customerBL.LoginCustomerAsync(model);
        ...
        public async Task<IActionResult> ForgotPassword([FromBody] ForgotPassword model)
        {
            try
            {
                var result = await _customerBL.ForgotPasswordAsync(model);
                if (result)
                {
                    var response = new ResponseModel<string>
                    {
        ...
        public async Task<IActionResult> ResetPassword([FromBody] ResetPasswordModel model)
        {
            try
            {
                var result = await _customerBL.ResetPasswordAsync(model);
                if (result)
                {
                    var response = new ResponseModel<string>
                    {
